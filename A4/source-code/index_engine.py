...
def main():
    ...
    with gzip.open(gz_file, 'rt') as f:
        ...
        for line in f:
            ...
            if "</DOC>" in line:
                in_doc = False
                build_index(doc_content, doc_id, index_folder)
                doc_id += 1
                doc_content = ""
    ...
    pickle.dump(metadata_dict, open(index_folder + '/pickles/metadata_dict.p', 'wb'))
    pickle.dump(id_to_docno, open(index_folder + '/pickles/id_to_docno.p', 'wb'))
    pickle.dump(docno_to_id, open(index_folder + '/pickles/docno_to_id.p', 'wb'))
    pickle.dump(termid_to_term, open(index_folder + '/pickles/termid_to_term.p', 'wb'))
    pickle.dump(term_to_termid, open(index_folder + '/pickles/term_to_termid.p', 'wb'))
    pickle.dump(postings, open(index_folder + '/pickles/postings.p', 'wb'))

# method to save the raw text of the doc
def save_doc(doc_content, date, doc_id, index_folder):
    ...
    with gzip.open(filename, 'wt') as doc_file:
...
def tokenize(text, headline='', graphic=''):
    ...
    headline_tokens = list(filter(None, headline_tokens))

    graphic_tokens = re.split(r"[^a-zA-Z0-9]", graphic)
    graphic_tokens = list(filter(None, graphic_tokens))
...
def build_index(doc_content, doc_id, index_folder):
    # extracting docno
    ...
    save_doc(doc_content, date, doc_id, index_folder)
